...
Usage:
  ...
  python3 plex_unwatched.py --plex-url http://192.168.1.10:32400 --plex-token abc123 \\
                             --tautulli-url http://192.168.1.10:8181 --tautulli-key xyz789
...
def get(session: requests.Session, url: str, params: dict | None = None, timeout: int = 30):
...
def tautulli_call(session: requests.Session, base_url: str, api_key: str,
                  cmd: str, extra: dict | None = None) -> dict:
...
def fetch_tautulli_history(session: requests.Session, base_url: str,
                           api_key: str) -> dict:
    """
    ...
    with Progress(SpinnerColumn(), TextColumn("{task.description}"),
        ...
        while True:
            data = tautulli_call(session, base_url, api_key, "get_history", {
                "length": page_size,
                "start": start,
            ...
            for rec in records:
                ...
                if rk not in history:
                    history[rk] = {"play_count": 0, "max_progress": 0.0, "last_watched": 0}
            ...
            progress.update(task, description=
                f"Fetching Tautulli history... {start}/{total}")
...
def plex_get(session: requests.Session, base_url: str, token: str,
             path: str, extra_params: dict | None = None) -> ET.Element:
...
def plex_fetch_all(session: requests.Session, base_url: str, token: str,
                   path: str, tag: str,
                   extra_params: dict | None = None,
                   progress: Progress | None = None,
                   description: str = "Fetching...") -> list[ET.Element]:
    ...
    while True:
        ...
        start += len(batch)
        if task is not None and progress:
            progress.update(task, completed=start,
                            description=f"{description} {start}/{total}")
...
def fetch_plex_libraries(session: requests.Session, base_url: str,
                         token: str) -> list[dict]:
...
def fetch_plex_movies(session: requests.Session, base_url: str,
                      token: str, library_key: str,
                      progress: Progress | None = None,
                      description: str = "Fetching movies...") -> list[dict]:
    """Fetch all movies from a Plex movie library."""
    movies = []
    for video in plex_fetch_all(session, base_url, token,
                                f"/library/sections/{library_key}/all", "Video",
...
def fetch_plex_episodes(session: requests.Session, base_url: str,
                        token: str, library_key: str,
                        progress: Progress | None = None,
                        description: str = "Fetching episodes...") -> list[dict]:
...
def fetch_all_plex_media(session: requests.Session, base_url: str,
                         token: str,
                         exclude_libraries: list[str] | None = None) -> tuple[list, list]:
    ...
    with Progress(SpinnerColumn(), TextColumn("{task.description}"),
        ...
        for lib in libraries:
            progress.update(lib_task,
                            description=f"Library: {lib['title']} ({lib['type']})")

            if lib["type"] == "movie":
                movies = fetch_plex_movies(
                    session, base_url, token, lib["key"],
                    progress=progress,
...
def analyze_movies(movies: list, history: dict,
                   min_size_bytes: int) -> list[dict]:
    results = []
    for m in movies:
        rk = str(m["rating_key"])
        h = history.get(rk, {})
...
def analyze_shows(episodes: list, history: dict,
                  min_size_bytes: int) -> list[dict]:
    shows: dict[str, dict] = {}
    ...
    for s in shows.values():
        ...
        s["pct_never_watched"] = (s["never_watched_count"] / total * 100) if total else 0.0
        s["unwatched_seasons"] = sorted(
            (
                season for season, counts in s["seasons"].items()
                if counts["total"] > 0
...
def render_movies_table(movies: list, top: int) -> None:
    shown = movies[:top]
    table = Table(
        title=f"Movies — All unwatched/partial/watched  (top {len(shown)} of {len(movies)} by size)",
        box=box.ROUNDED,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--plex-url",      metavar="URL",
        help="Plex base URL, e.g. http://192.168.1.10:32400 or https://plex.example.com")
    parser.add_argument("--plex-token",    metavar="TOKEN",
        help="Plex authentication token (X-Plex-Token)")
    parser.add_argument("--tautulli-url",  metavar="URL",
        help="Tautulli base URL, e.g. http://192.168.1.10:8181")
    parser.add_argument("--tautulli-key",  metavar="KEY",
    ...
    parser.add_argument("--min-size",      type=int, default=DEFAULT_MIN_SIZE_MB, metavar="MB",
    ...
    parser.add_argument("--movie-max-plays",      type=int, metavar="N", default=None,
        help="Only show movies with N or fewer total plays (e.g. 0=never watched, 2=watched ≤2 times)")
    parser.add_argument("--show-max-watched-pct", type=int, metavar="N", default=None,
        help="Only show TV shows where at most N%% of episodes have been watched (e.g. 10)")
    parser.add_argument("--ignore-recent-days", type=int, metavar="N", default=None,
        help="Exclude media with any watch activity in the last N days (e.g. 365 for 1 year)")
    parser.add_argument("--exclude-library", action="append", dest="exclude_libraries",
        metavar="NAME",
...
def main() -> None:
    ...
    console.print(f"[bold]Watched threshold:[/]         {int(WATCHED_THRESHOLD * 100)}% progress = watched")
    console.print(f"[bold]Unwatched season threshold:[/] all eps unwatched = season flagged")
    console.print(f"[bold]Top results:[/]               {args.top}")
    console.print(f"[bold]Env file:[/]                  {ENV_FILE} ({'found' if ENV_FILE.exists() else 'not found'})")
    if args.min_size:
    ...
    if args.show_max_watched_pct is not None:
        console.print(f"[bold]Show max watched:[/]          {args.show_max_watched_pct}%")
    if args.ignore_recent_days is not None:
        console.print(f"[bold]Ignore recent activity:[/]    last {args.ignore_recent_days} days")
    ...
    # Analyze
    movies = [] if args.shows_only  else analyze_movies(raw_movies, history, min_size_bytes)
    shows  = [] if args.movies_only else analyze_shows(raw_episodes, history, min_size_bytes)
    ...
    if args.show_max_watched_pct is not None:
        shows = [
            ...
            and (s["fully_watched_count"] / s["total_episodes"] * 100) <= args.show_max_watched_pct
        ]
